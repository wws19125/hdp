...
public abstract class OutputCommitter {
   ...
   * called multiple times, once per job attempt.  Job初始化的时候设置job,job会尝试调用多次
   * 
   ...
   * multiple times.   为完成的job收尾，也可能会被调用多次
   * 
   * @param jobContext Context of the job whose output is being written.
   * @throws IOException
   ...
   * @throws IOException
   */
    public void commitJob(JobContext jobContext) throws IOException { //提交job
   ...
   */
    public abstract void commitTask(TaskAttemptContext taskContext) //提交task,从task进程
  throws IOException;
