...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
  ...
  private static final PathFilter hiddenFileFilter = new PathFilter(){
	  ...
	  String name = p.getName();  //获取文件名
	  return !name.startsWith("_") && !name.startsWith(".");  //判断是否以_或者.开头
      }
    }; 
  ...
  protected List<FileStatus> listStatus(JobContext job
    ...
    // get tokens for all the required FileSystems..           这里不明白
    ...
    for (int i=0; i < dirs.length; ++i) {
      ...
      FileStatus[] matches = fs.globStatus(p, inputFilter); //进行筛选
      if (matches == null) {
	    ...
	    if (globStat.isDirectory()) {  //判断是个目录
            RemoteIterator<LocatedFileStatus> iter =
                fs.listLocatedStatus(globStat.getPath());  //可能是远程文件
            while (iter.hasNext()) { //遍历
              LocatedFileStatus stat = iter.next();
              if (inputFilter.accept(stat.getPath())) {
		  if (recursive && stat.isDirectory()) { //第归
  ...
  protected void addInputPathRecursively(List<FileStatus> result,
					 FileSystem fs, Path path, PathFilter inputFilter)  //第归调用，遍历
      throws IOException {
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
      long minSize = Math.max(getFormatMinSplitSize(), getMinSplitSize(job)); //获取默认的最小的inputSplits数量。
      long maxSize = getMaxSplitSize(job); //获取配置中的参数
  ...
  public static void setInputPaths(Job job, 
    ...
    conf.set(INPUT_DIR, str.toString()); //从这里就可以看到，配置参数使用的是,隔开多个参数
  }
  ...
  private static String[] getPathStrings(String commaSeparatedPaths) { //分离多个路径，路径之间用,隔开
  ...
  public static Path[] getInputPaths(JobContext context) {
      String dirs = context.getConfiguration().get(INPUT_DIR, "");  //可以在mp程序中找到 setInputPaths
      String [] list = StringUtils.split(dirs); //分割,以,为分隔符号
