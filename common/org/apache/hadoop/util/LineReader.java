  ...
  throws IOException {
    ...
    do {
      ...
      if (bufferPosn >= bufferLength) {
        ...
        bufferLength = in.read(buffer); //只有当bufferPosn>=bufferLength才去读取，同时初始化startPosn
        if (bufferLength <= 0) {
          break; // EOF
        }
      }
  ...
  private int readCustomLine(Text str, int maxLineLength, int maxBytesToConsume)
    ...
    */
    ...
    do {
      ...
      for (; bufferPosn < bufferLength; ++bufferPosn) { //用户自定义的分界符号可能是一个字符串，所以要一个一个字符对比才能知道，如下所示
      ...
      int readLength = bufferPosn - startPosn;
      bytesConsumed += readLength;
      int appendLength = readLength - delPosn; //要增加的长度
      if (appendLength > maxLineLength - txtLength) {
        appendLength = maxLineLength - txtLength;
      }
      if (appendLength > 0) {
